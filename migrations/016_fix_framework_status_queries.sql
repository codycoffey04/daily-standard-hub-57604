-- Fix framework_status queries to join with entry_status view
-- The framework_status field is calculated in the entry_status view, not stored in daily_entries table

-- Drop and recreate get_monthly_summary function with proper entry_status join
...
BEGIN
  ...
  SELECT 
    ...
    -- Framework status counts from entry_status view
    COUNT(*) FILTER (WHERE es.framework_status = 'TOP')::integer as top_framework_entries,
    COUNT(*) FILTER (WHERE es.framework_status = 'BOTTOM')::integer as bottom_framework_entries,
    COUNT(*) FILTER (WHERE es.framework_status = 'OUTSIDE')::integer as outside_framework_entries,
    
    -- Framework compliance percentage
    CASE 
      WHEN COUNT(*) > 0 
      THEN ROUND(
        (COUNT(*) FILTER (WHERE es.framework_status = 'TOP')::numeric / COUNT(*)::numeric) * 100, 
        1
      )
      ELSE 0 
  ...
  LEFT JOIN entry_status es ON es.entry_id = de.id  -- Join with entry_status view for framework_status
