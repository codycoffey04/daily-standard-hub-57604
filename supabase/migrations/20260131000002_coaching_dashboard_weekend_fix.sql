-- Fix: Show most recent week's episode instead of strictly current week
-- This handles weekends when no current week episode exists yet
...
DECLARE
  ...
  v_result JSON;
BEGIN
  -- Find most recent published episode for this member (within last 14 days)
  -- This handles weekends when current week episode doesn't exist yet
  IF p_is_csr THEN
  ...
  -- If no recent episode, return null
  IF v_current_episode.id IS NULL THEN
    RETURN NULL;
  END IF;

  -- Find previous week's episode (relative to the episode we found)
  IF p_is_csr THEN
    ...
    WHERE csr_profile_id = p_member_id
      AND coaching_type = p_coaching_type
      AND status = 'published'
      AND week_start = v_current_episode.week_start - INTERVAL '7 days'
...
COMMENT ON FUNCTION public.get_coaching_score_comparison IS 'Returns coaching scores for most recent week (within 14 days) and previous week with deltas for dashboard card display. Handles weekends when current week episode does not exist yet.';
