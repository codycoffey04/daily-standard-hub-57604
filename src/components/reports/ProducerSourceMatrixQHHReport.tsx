...
export const ProducerSourceMatrixQHHReport: React.FC<ProducerSourceMatrixQHHReportProps> = ({
  ...
  const combinations = useMemo(() => rows.filter(item => item.qhh > 0).length, [rows])
  ...
  const exportToCSV = useCallback(() => {
    ...
    const producerTotals = producers.map(producer => ({
      producer,
      total: rows.filter(d => d.producer_name === producer).reduce((sum, d) => sum + d.qhh, 0)
    }))

    const sourceTotals = sources.map(source => ({
      source,
      total: rows.filter(d => d.source_name === source).reduce((sum, d) => sum + d.qhh, 0)
    }))

    const grandTotal = rows.reduce((sum, d) => sum + d.qhh, 0)
  ...
  return (
    <div className="space-y-6">
      ...
      <Card>
        ...
        <CardContent>
          <ProducerSourceMatrixQHHChart data={rows} height={500} />
        </CardContent>
      </Card>
    </div>
  )
