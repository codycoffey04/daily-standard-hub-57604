...
interface ProducerSourceMatrixQuotesChartProps {
  data: ProducerSourceMatrixData[]
  height?: number
}

export const ProducerSourceMatrixQuotesChart: React.FC<ProducerSourceMatrixQuotesChartProps> = ({ 
  data, 
  height = 400 
}) => {
  ...
  const getBackgroundColor = (intensity: number): string => {
    ...
    return `hsla(var(--chart-2) / ${opacity})`
  }
  ...
  const exportToCSV = () => {
    ...
    link.href = url
    link.download = 'producer-source-quotes-matrix.csv'
